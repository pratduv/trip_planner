...
def process_message(message: str, conversation_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    openai_messages = [
        {"role": "system", "content": "You are a helpful travel assistant. Your job is to help users plan their trips by understanding their preferences, suggesting destinations, and providing travel advice. Keep your responses concise and focused on travel planning. At the end of your response, suggest 3-4 relevant follow-up options the user might be interested in."}
    ]
    ...
    try:
        ...
        suggestion_prompt = [
            {"role": "system", "content": "Based on the conversation between a travel assistant and a user, generate 3-4 short phrases (2-4 words each) that would be good clickable suggestion buttons for the user to continue the conversation. Format as a JSON array of strings. Only include the JSON array, nothing else."}
        ]
        ...
        if len(conversations[conversation_id]["messages"]) >= 4:  # After a few exchanges
            preference_prompt = [
                {"role": "system", "content": "Extract travel preferences from this conversation. Return ONLY a JSON array of strings representing travel preferences (e.g., [\"beaches\", \"cultural sites\", \"food\"])."}
            ]
